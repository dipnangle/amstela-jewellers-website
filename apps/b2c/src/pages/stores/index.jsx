...
const STORES = [
  ...
  },
  {
    ...
    address: 'B-10, Zaveri Bazaar, Kalbadevi, Mumbai, Maharashtra — 400 002',
    phone: '+91 98XXX XXXXX',
  ...
  return (
    <main>
      <div className="page-head bg-velvet">
        ...
        <div className="wrap">
          <div className="crumb">
            <Link to="/">Home</Link><span className="sep">/</span><span>Stores</span>
          </div>
          <h1>Our Boutiques</h1>
          <p className="lede">Visit us for a private showing, custom consultation, or simply to experience Amstela in person.</p>
        </div>
      </div>
          ...
          {STORES.map((store, i) => (
            <div key={store.id} className={`coll ${i % 2 !== 0 ? 'flip' : ''}`}>
              ...
              <div className="coll-body">
                {store.badge && <span className="prod-badge" style={{ position: 'static', display: 'inline-block', marginBottom: 14 }}>{store.badge}</span>}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 20, marginBottom: 28 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <a href={`tel:${store.phone}`} style={{ fontSize: 14, color: 'var(--body)' }}>{store.phone}</a>
                  </div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                ...
                <Link to="/contact" className="link-gold">Book a visit</Link>
              </div>
            </div>
          ))}
        </div>
