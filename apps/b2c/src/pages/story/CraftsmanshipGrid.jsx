    ...
    <section ref={ref} className="py-24 lg:py-40 bg-white">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-8">
        <div className="text-center mb-16 lg:mb-24 transition-all duration-1000">
          <p className="font-sans text-[10px] font-bold tracking-[0.4em] uppercase text-[#B8862A] mb-4">The Process</p>
          <h2 className="font-serif text-4xl sm:text-5xl lg:text-6xl font-light text-[#0D1527] leading-tight">Art of the <span className="italic text-[#B8862A]">Masterpiece</span></h2>
        </div>
          ...
          {STEPS.map((step, i) => (
            ...
            >
              <div className="relative aspect-[4/5] overflow-hidden bg-gray-50 shadow-md">
                <img src={step.image} alt={step.title} className="w-full h-full object-cover transition-transform duration-[2s] hover:scale-105" />
                <div className="absolute top-6 left-6 w-10 h-10 bg-white/90 backdrop-blur-sm flex items-center justify-center font-serif text-lg text-[#0D1527] shadow-sm">{step.num}</div>
              </div>
              <div className="space-y-4">
                <h3 className="font-serif text-2xl lg:text-3xl font-light text-[#0D1527] tracking-tight">{step.title}</h3>
                <p className="font-sans text-sm text-[#555555] leading-[1.8] tracking-wide">{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
