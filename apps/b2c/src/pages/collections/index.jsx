...
const CATEGORIES = ['Rings', 'Earrings', 'Necklaces', 'Mangalsutra', 'Bracelets', 'Bridal Sets']
...
export default function CollectionsPage() {
  ...
  const activeMetals = useMemo(() => searchParams.get('metal')?.split(',').filter(Boolean) || [], [searchParams])
  const activeSort = searchParams.get('sort') || 'featured'

  const activeCollection = slug ? collectionsData.find(c => c.slug === slug) : null
  ...
  const baseByCollection = useMemo(() => {
    return slug ? productsData.filter(p => p.collectionSlug === slug) : productsData
  }, [slug])
  ...
  const filtered = useMemo(() => {
    ...
    if (activeMetals.length > 0) {
      base = base.filter(p => activeMetals.some(m => p.metal.toLowerCase().includes(m.toLowerCase())))
    }
    ...
    <main>
      ...
      <div className="page-head bg-velvet">
        ...
        <div className="wrap">
          ...
          <h1>{activeCollection ? activeCollection.name : 'All Masterpieces'}</h1>
          {activeCollection && (
      ...
      <div className="section wrap">
        <div className="listing">
          ...
          <aside className="filters">
            <div className="filter-header" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <SlidersHorizontal size={18} />
              <h4 style={{ margin: 0, fontFamily: 'var(--sans)', fontSize: 14, fontWeight: 700, letterSpacing: '0.1em' }}>FILTERS</h4>
            </div>
              ...
              {CATEGORIES.map(cat => {
                ...
                return (
                  <label 
                    ...
                    style={{ opacity: count === 0 && !isActive ? 0.5 : 1, pointerEvents: count === 0 && !isActive ? 'none' : 'auto' }}
                  >
                    <span className="box">
                    ...
                    {cat}
                    <span className="count" style={{ marginLeft: 'auto', fontSize: 11, opacity: 0.6 }}>({count})</span>
                  </label>
                )
              })}
            </div>
            ...
            <div className="filter-group">
              <h5>Collection</h5>
              <Link to="/collections" className={`chk ${!slug ? 'on' : ''}`}>
                <span className="box">
                  {!slug && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                </span>
                All Collections
              </Link>
              {collectionsData.map(c => (
                <Link key={c.slug} to={`/collections/${c.slug}?${searchParams.toString()}`} className={`chk ${slug === c.slug ? 'on' : ''}`}>
                  <span className="box">
                    {slug === c.slug && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                  </span>
                  {c.name}
                </Link>
              ))}
            ...
            {(activeCategories.length > 0 || activeMetals.length > 0 || slug) && (
              <button 
                onClick={clearAll}
                className="btn btn-ghost btn-block" 
                style={{ marginTop: 20, fontSize: 11, padding: '12px' }}
              >
                Clear All Filters
              </button>
            )}
          ...
          <div>
            <div className="listing-toolbar">
                ...
                <div className="chips">
                  {activeCategories.map(cat => (
                    <span key={cat} className="chip">
                      {cat}
                      <button onClick={() => toggleFilter('category', cat)}><X size={12} /></button>
                    </span>
                  ))}
                  {activeMetals.map(m => (
                    <span key={m} className="chip">
                      {m}
                      <button onClick={() => toggleFilter('metal', m)}><X size={12} /></button>
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', color: 'var(--body)' }}>SORT:</span>
                <select 
                  className="select" 
                  value={activeSort} 
                  onChange={(e) => setSort(e.target.value)}
                  style={{ padding: '8px 12px', border: '1px solid var(--border)', fontSize: 12 }}
                >
            ...
            {filtered.length > 0 ? (
              ...
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'var(--offwhite)', borderRadius: 4 }}>
                <p style={{ fontFamily: 'var(--serif)', fontSize: 26, color: 'var(--body)', marginBottom: 16 }}>
                  No pieces match your selection
                </p>
                <button className="link-gold" onClick={clearAll}>
  ...
  return (
    ...
    >
      <div className="prod-thumb zoom">
        {product.badge && (
          <span className={`prod-badge ${['Sale', 'New'].includes(product.badge) ? 'sale' : ''}`}>{product.badge}</span>
        )}
        <button
        ...
        <div className="prod-actions">
          <Link to={`/product/${product.slug}`} className="btn btn-navy">View</Link>
          <Link to="/contact" className="btn btn-gold">Enquire</Link>
        </div>
      </div>
      <div className="prod-info">
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, marginTop: 4 }}>
           ...
           <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>₹{product.price.toLocaleString()}</p>
        </div>
      </div>
    </article>
  )
