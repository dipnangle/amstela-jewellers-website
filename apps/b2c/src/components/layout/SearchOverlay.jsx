...
export default function SearchOverlay({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    else { setQuery(''); document.body.style.overflow = '' }
    ...
    <div className="search-overlay" role="dialog" aria-modal="true" aria-label="Search">
      ...
      <div className="search-panel">
        ...
        <div className="search-row">
          <Search size={20} className="search-icon-prefix" strokeWidth={1.5} />
          <input
            ref={inputRef}
            type="search"
            value={query}
        ...
        <div className="search-body">
          {results.length > 0 ? (
            <div className="search-results">
              <p className="search-label">Results for "{query}"</p>
              {results.map((p) => (
                <Link key={p.id} to={`/product/${p.slug}`} className="search-result-row" onClick={onClose}>
                  <img src={p.images[0]} alt={p.name} className="search-thumb" />
                  <div>
                    <p className="search-result-collection">{p.collectionName}</p>
                    <p className="search-result-name">{p.name}</p>
                    <p className="search-result-meta">{p.metal} · {p.carat}</p>
                  </div>
                  <ArrowRight size={14} className="search-result-arrow" strokeWidth={1.5} />
                </Link>
              ))}
            </div>
          ) : (
            <div className="search-empty">
              ...
              <div className="search-quick">
                {QUICK_LINKS.map((l) => (
                  <Link key={l.href} to={l.href} className="search-quick-link" onClick={onClose}>
                    {l.label}
                  </Link>
                ))}
              </div>
