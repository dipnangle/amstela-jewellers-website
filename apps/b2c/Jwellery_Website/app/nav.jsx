            ...
            <div className="mega-grid">
                {Object.entries(MEGA).map(([head, items]) => (
                    <div className="mega-col" key={head}>
                        <h5>{head}</h5>
                        {items.map((it) => (
                            <a key={it} onClick={() => go("/collections")}>
                                {it}
                            </a>
                        ))}
                    </div>
        ...
        <React.Fragment>
            <header className={`nav ${solid ? "is-solid" : "is-top"}`}>
                <div className="announce">
                    ...
                    <b>Book a private consultation</b>
                </div>
                <div className="nav-inner">
                    <nav className="nav-left">
                        <span
                            className="nav-link has-mega"
                            onMouseEnter={() => setMega(true)}
                            onClick={() => nav("/collections")}
                        >
                            Collections
                        </span>
                        <RLink className="nav-link" to="/collections">
                            Diamond
                        </RLink>
                        <RLink className="nav-link" to="/collections">
                            Gold
                    ...
                    <div className="nav-right">
                        <nav className="nav-left" style={{ marginRight: 8 }}>
                            <RLink className="nav-link" to="/about">
                                About
                            </RLink>
                            <RLink className="nav-link" to="/manufacturing">
                                Manufacturing
                            </RLink>
                            <RLink className="nav-link" to="/contact">
                                Contact
                            </RLink>
                        </nav>
                        <button
                        ...
                        <button className="nav-ico" aria-label="Account">
                            <IcoUser size={20} />
                        </button>
                        <button
                            className="nav-ico"
            ...
            {/* mobile drawer */}
            <div
                className={`drawer-scrim ${drawer ? "open" : ""}`}
            ...
            >
                ...
                <nav className="drawer-nav">
                    {[
                        ...
                        <RLink key={l} to={t} onClick={() => setDrawer(false)}>
                            {l}
                            <span>
                                <IcoChevR size={18} />
                            </span>
